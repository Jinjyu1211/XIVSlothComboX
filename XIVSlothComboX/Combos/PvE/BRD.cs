...
namespace XIVSlothComboX.Combos.PvE
{
    internal static class BRD
    {
        ...
        internal static bool SongIsNotNone(Song        value) => (int)value != 0;
        internal static bool SongIsNone(Song           value) => (int)value == 0;
        internal static bool SongIsWandererMinuet(Song value) => (int)value == 1;
        ...
        internal class BRD_AoE_oGCD : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                {
                    BRDGauge? gauge = GetJobGauge<BRDGauge>();
                    bool songWanderer = (int)gauge.Song == 1;
        ...
        internal class BRD_AoE_AdvMode : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Ladonsbite or QuickNock)
                {
                    ...
                    bool songNone = (int)gauge.Song == 0;
                    bool songWanderer = (int)gauge.Song == 1;
                    bool songMage = (int)gauge.Song == 2;
                    bool songArmy = (int)gauge.Song == 3;
        ...
        internal class BRD_OneButtonSongs : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is 放浪神的小步舞曲WanderersMinuet)
                {
                    ...
                    if (ActionReady(放浪神的小步舞曲WanderersMinuet) || ((int)gauge.Song == 1 && songTimerInSeconds > 11))
                        return 放浪神的小步舞曲WanderersMinuet;

                    if (ActionReady(贤者的叙事谣MagesBallad) || ((int)gauge.Song == 2 && songTimerInSeconds > 2))
                        return 贤者的叙事谣MagesBallad;

                    if (ActionReady(军神的赞美歌ArmysPaeon) || ((int)gauge.Song == 3 && songTimerInSeconds > 2))
                        return 军神的赞美歌ArmysPaeon;
